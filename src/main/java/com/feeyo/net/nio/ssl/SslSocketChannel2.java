...
public class SslSocketChannel2 implements ByteChannel, WrappedByteChannel, SslChannel {
  ...
  //
  private final Logger LOGGER = LoggerFactory.getLogger(SslSocketChannel2.class);
  ...
  protected void createBuffers(SSLSession session) {
    ...
    if (inData.remaining() != 0 && LOGGER.isTraceEnabled()) {
      LOGGER.trace(new String(inData.array(), inData.position(), inData.remaining()));
    }
    inData.rewind();
    inData.flip();
    if (inCrypt.remaining() != 0 && LOGGER.isTraceEnabled()) {
      LOGGER.trace(new String(inCrypt.array(), inCrypt.position(), inCrypt.remaining()));
    }
    inCrypt.rewind();
    inCrypt.flip();
    outCrypt.rewind();
