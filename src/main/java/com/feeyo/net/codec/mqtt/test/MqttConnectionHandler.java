...
import static com.feeyo.net.codec.mqtt.MqttMessageIdVariableHeader.from;
...
public class MqttConnectionHandler implements NIOHandler<MqttConnection> {
	...
	public void handleReadEvent(MqttConnection con, byte[] data) throws IOException {
		try {
			List<MqttMessage> msgs = decoder.decode(data);
			for(MqttMessage msg: msgs) 
	...
	//
	private void handleMQTTMessage(MqttConnection con, MqttMessage msg) {
	...
	private void processPubRec(MqttConnection connection, MqttMessage msg) {
        final int messageId = ((MqttMessageIdVariableHeader) msg.variableHeader()).messageId();
	}
	
	private void processPubComp(MqttConnection connection, MqttMessage msg) {
        final int messageId = ((MqttMessageIdVariableHeader) msg.variableHeader()).messageId();
	}
	
	private void processPubRel(MqttConnection connection, MqttMessage msg) {
		final int messageId = ((MqttMessageIdVariableHeader) msg.variableHeader()).messageId();
	}
	
	private void processDisconnect(MqttConnection connection, MqttMessage msg) {
		connection.close("received DISCONNECT!");
	}
	
	private void processPubAck(MqttConnection connection, MqttMessage msg) {
		final int messageId = ((MqttMessageIdVariableHeader) msg.variableHeader()).messageId();
	}
