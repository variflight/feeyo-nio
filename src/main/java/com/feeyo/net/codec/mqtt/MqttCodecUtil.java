...
final class MqttCodecUtil {
    ...
    static MqttFixedHeader validateFixedHeader(MqttFixedHeader fixedHeader) throws UnknownProtocolException {
    ...
    static MqttFixedHeader resetUnusedFields(MqttFixedHeader fixedHeader) {
        switch (fixedHeader.messageType()) {
            case CONNECT:
            case CONNACK:
            case PUBACK:
            ...
            case DISCONNECT:
				if (fixedHeader.isDup() || fixedHeader.qosLevel() != MqttQoS.AT_MOST_ONCE || fixedHeader.isRetain()) 
					return new MqttFixedHeader(fixedHeader.messageType(), false, MqttQoS.AT_MOST_ONCE, false, fixedHeader.remainingLength());
				//
                return fixedHeader;
            case PUBREL:
            case SUBSCRIBE:
            case UNSUBSCRIBE:
				if (fixedHeader.isRetain()) 
					return new MqttFixedHeader(fixedHeader.messageType(), fixedHeader.isDup(), fixedHeader.qosLevel(), false, fixedHeader.remainingLength());
				//
                return fixedHeader;
            default:
                return fixedHeader;
