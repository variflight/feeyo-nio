 ...
 */
public class MqttDecoder implements Decoder<List<MqttMessage>> {
    //
    private enum State {
        READ_FIXED_HEADER,
    ...
    //
    @Override
	public List<MqttMessage> decode(byte[] data) throws UnknownProtocolException {
    	//
		...
		//
		List<MqttMessage> out = new ArrayList<>();
		//
		int off0 = offset;
		...
		try {
			...
			for(;;) {
				...
				case READ_PAYLOAD:
					...
					//
					//
					MqttMessage message = MqttMessageFactory.newMessage(fixedHeader, variableHeader, payloadResult);
					out.add(message);
					//
					// reset
					fixedHeader = null;
    ...
    private MqttFixedHeader decodeFixedHeader(CompositeByteArray buffer) throws UnknownProtocolException {
    	// read 1 byte
        short b1 = (short) (buffer.get( offset++ ) & 0xFF);
        //
        MqttMessageType messageType = MqttMessageType.valueOf(b1 >> 4);		// 报文类型
	...
	private Object decodeVariableHeader(CompositeByteArray buffer, MqttFixedHeader fixedHeader)
	...
	private MqttMessageIdVariableHeader decodeMessageIdVariableHeader(CompositeByteArray buffer)
			throws UnknownProtocolException {
	...
	private MqttPublishVariableHeader decodePublishVariableHeader(CompositeByteArray buffer,
			MqttFixedHeader fixedHeader) throws UnknownProtocolException {
		//
    ...
    //
	// 解析有效载荷 Payload
	private Object decodePayload(CompositeByteArray buffer, MqttMessageType messageType, int remainingLength, Object variableHeader)
