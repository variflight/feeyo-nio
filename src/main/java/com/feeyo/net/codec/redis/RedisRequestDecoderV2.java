...
public class RedisRequestDecoderV2 implements Decoder<List<RedisRequest>> {
    ...
    private State readState = State.READ_SKIP; // 读取状态
    //
    private enum State {
        READ_SKIP, // 跳过空格
        READ_INIT, // 开始
    ...
    public List<RedisRequest> decode(byte[] buffer) throws UnknownProtocolException {
        ...
        //
        try {
            int argIndex = -1; // 读取到的参数索引
            int argCount = 0; // 参数的数量
            int argLength = 0; // 参数的长度
            //
            decode:
            for (; ; ) {
                switch (readState) {
                    ...
                    case READ_INIT: {
                        ...
                        if (commandBeginByte == '*') {
                            readOffset++;
                            readState = State.READ_ARG_COUNT;
                        } else if (commandBeginByte == '$') {
                            readOffset++;
                            readState = State.READ_ARG_LENGTH;
                        }
                        break;
                    }
                    case READ_ARG_COUNT: {
                        ...
                        this.readState = State.READ_INIT;
                        break;
                    }
                    ...
                    case READ_END: {
                        ...
                        if (compositeArray.getByteCount() < readOffset) {
                            ...
                            argLength = 0;
                            pipeline.add(request);
                            this.readState = State.READ_SKIP;
                        }
                    }
                    break;
                    default:
                        throw new UnknownProtocolException("Unknown state: " + readState);
                }
            }
