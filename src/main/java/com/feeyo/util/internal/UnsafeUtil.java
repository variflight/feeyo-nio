...
public final class UnsafeUtil {
	...
	/**
	 * Returns the offset of the provided class and fieldName, or {@code -1} if
	 * {@code sun.misc.Unsafe} is not available.
	 */
	...
	/**
	 * Returns a new {@link String} backed by the given {@code chars}. The char
	 * array should not be mutated any more after calling this function.
	 */
	public static String moveToString(final char[] chars) {
		if (STRING_VALUE_OFFSET == -1) {
			// In the off-chance that this JDK does not implement String as we'd
			// expect, just do a copy.
		...
		try {
			...
			// This should never happen, but return a copy as a fallback just in
			// case.
			return new String(chars);
		}
	...
	public static class UnsafeAccessor {
		...
		/**
		 * Reports the offset of the first element in the storage allocation of
		 * a given array class.
		 */
