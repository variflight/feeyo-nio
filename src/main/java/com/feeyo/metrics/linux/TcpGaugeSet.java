...
public class TcpGaugeSet extends MetricSetCollect {

    private static final Logger LOGGRE = LoggerFactory.getLogger(SystemMemoryGaugeSet.class);
    ...
    public TcpGaugeSet(String filePath, long dataTTL, TimeUnit unit) {
        super(dataTTL, unit);
        //
    ...
    protected void collect() {
        try {
            ...
            LOGGRE.warn("Error during reading file {}", filePath, e);
        }
    }
    ...
    private void populateGauges() {
        for (int i = 0; i < METRICS.length; i++) {
            gauges.put(METRICS[i], new TcpGauge(i));
        }
        // add tcp.current_estab
        gauges.put(CURRENT_ESTAB, new Gauge<Long>() {
            @Override
        ...
        gauges.put(RETRAN_RATIO, new Gauge<Double>() {
            @Override
    ...
    private class TcpGauge implements Gauge<Double> {
